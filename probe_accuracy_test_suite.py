...
DATA_DIR = "~/printer_data/logs"  # Updated the data directory path
...
class Probe():
    ...
    def check_error(msg):
        error_messages = {
            "klicky_macro_issue": "Must perform PROBE_ACCURACY with the probe above the BED!"
        }

        if error_messages["klicky_macro_issue"] in msg:
...
class Printer:
    ...
    def move(self, x=None, y=None, z=None, feedrate=99999, echo=False):
        if z is None:
    ...
    def _move(self, x=None, y=None, z=None, feedrate=None, echo=False):
        gcode_cmd = "G0"
        if x is not None:
            gcode_cmd += f" X{x}"
        if y is not None:
    ...
    def _get_bed_center(self) -> Tuple:
        axis_min = numpy.array(self.query("toolhead", "axis_minimum")[:2])
        axis_max = numpy.array(self.query("toolhead", "axis_maximum")[:2])

        center = numpy.mean([axis_min, axis_max], axis=0)
        return tuple(center)
    ...
    def _get_random_loc(self, max_range=50):
        ...
        random_loc = numpy.random.random(2) * (axis_max - axis_min - 2 * max_range) + max_range + axis_min
        return tuple(random_loc)

    def _get_bed_corners(self) -> List:
        try:
            probe_config = self.config.get("probe", {})
            idm_config = self.config.get("idm", {})
            cartographer_config = self.config.get("cartographer", {})
            beacon_config = self.config.get("beacon", {})

            x_offset = probe_config.get("x_offset", 0) or \
                       idm_config.get("x_offset", 0) or \
                       cartographer_config.get("x_offset", 0) or \
                       beacon_config.get("x_offset", 0)

            y_offset = probe_config.get("y_offset", 0) or \
                       idm_config.get("y_offset", 0) or \
                       cartographer_config.get("y_offset", 0) or \
                       beacon_config.get("y_offset", 0)
        except:
            x_offset = 0
            y_offset = 0

        bed_mesh_config = self.config.get("bed_mesh", {})
        xmin, ymin = map(float, re.findall(r"[\d.]+", bed_mesh_config.get("mesh_min", "0 0")))
        xmax, ymax = map(float, re.findall(r"[\d.]+", bed_mesh_config.get("mesh_max", "0 0")))
...
class Test_suite():
    ...
    def run(self):
        ...
        file_nm = f"{RUNID}_{timestamp}_probe_accuracy_test"
        if self.export_csv:
            suiteframe.to_csv(
    ...
    def clean_old_data(self):
        file_pattern = os.path.join(self.output_dir, f"{RUNID}_*")
    ...
    def test_corner(self):
        ...
        dataframes = []
        bed_corners = self.printer.bed_corners

        for i, xy in enumerate(bed_corners):
        ...
        for i in range(self.repeatability):
            ...
            for j in range(4):
                ...
                dataframe = self._test_probe(
                    ...
                    testname=f"{j + 1:02d}: center {probe_count} samples"
                )
                dataframe["measurement"] = f"Test #{j + 1:02d}"
                dataframes.append(dataframe)
        print("Done")
    ...
    def _speedcheck(self, speeds):
        ...
        if speeds["stop"] >= 35:
            print(f"Warning: your maximum speeds will be {speeds['stop']}")
            confirm = None
...
def fetch_repo():
    ...
    os.chdir(repo_path)
    output = subprocess.run(["git", "pull"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
